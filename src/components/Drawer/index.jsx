...
const CustomDrawer = ({
  ...
  return <Drawer anchor="right" open={isDrawerOpen} onClose={toggleDrawer("right", false)}>
      <Box sx={styles.drawerContainer} id="drawer-container">
        ...
        {showContactsInfo ? <Box sx={styles.contactListInfo}>
          ...
          md: 3
              ...
              <Grid item xs={12}>
                <Card title="Sales Team" icon={ContactPageIcon} showContactsInfo={false}>
                  contents
                </Card>
              </Grid>

              <Grid item xs={12}>
                <Card title="Marketing Team" icon={ContactPageIcon} showContactsInfo={false}>
                  contents
                </Card>
              </Grid>
          ...
          </Box> : <Box id="edit-contact" sx={{
        flexGrow: 1
      }}>
            <Box sx={styles.editEmail}>
              <Typography fontWeight={600}>Email ID</Typography>
              {emailFields.map((email, index) => <TextField fullWidth label="" id="fullWidth" placeholder="eg. salesteam@br.in" sx={{
            mt: 2
          }} onChange={e => handleChangeEmail(e, index)} value={email} />)}
            ...
            <Box mt={3}>
              ...
              {contactFields.map(field => <TextField fullWidth label="" id="fullWidth" placeholder="eg. 8519701740" sx={{
            mt: 2
          }} />)}
        ...
        <Box id="button" sx={styles.saveButton}>
          <PrimaryButton backgroundColor="#BE212A" title="Save" width="100%" height="50px" borderRadius="10px" onClick={handleSaveContact} />
        </Box>
      </Box>
    </Drawer>;
};
